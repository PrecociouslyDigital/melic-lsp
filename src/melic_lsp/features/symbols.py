"""Document outline: sections, each labelled with its syllable profile.

The profile is the point. "Verse 1 · 8,6,8,6σ" tells you the shape of a section
from the outline alone, which is exactly what you want when checking whether verse
three still matches the tune you wrote for verse one.
"""
...
from ..signature import count_label

MAX_PROFILE_LINES = 8


def build(sections: list[Section], analyses: list[LineAnalysis]) -> list[lsp.DocumentSymbol]:
    by_row = {analysis.line.row: analysis for analysis in analyses}
    return [_symbol(section, by_row) for section in sections]


def _symbol(
    section: Section, by_row: dict[int, LineAnalysis]
) -> lsp.DocumentSymbol:
    ...
    return lsp.DocumentSymbol(
        name=section.title,
        detail=f"{profile}σ" if profile else "",
